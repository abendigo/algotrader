...
export const strategyMeta = {
  ...
  configFields: {
    common: {
      ...
      stopRangeFraction: { label: "Stop (range fraction)", type: "number" as const, default: 1.0, min: 0, step: 0.1 },
      trailActivateFraction: { label: "Trail activate (range fraction)", type: "number" as const, default: 0.5, min: 0, step: 0.1 },
      trailDistanceFraction: { label: "Trail distance (range fraction)", type: "number" as const, default: 0.3, min: 0, step: 0.1 },
      skipDays: { label: "Skip days (0=Sun,5=Fri)", type: "text" as const, placeholder: "5" },
    },
    backtest: {
      rewardRatio: { label: "Reward ratio", type: "number" as const, default: 0, min: 0, step: 0.5 },
    },
    live: {},
  },
};
...
export class LondonBreakoutStrategy implements Strategy {
  ...
  async init(ctx: StrategyContext): Promise<void> {
    // Reconcile with broker: adopt any positions that exist on OANDA
    // (e.g., from a previous session that crashed before closing them)
    const brokerPositions = await ctx.broker.getPositions();
    for (const bp of brokerPositions) {
      if (!(this.instruments as readonly string[]).includes(bp.instrument)) continue;
      if (this.positions.has(bp.instrument)) continue;

      // We don't know the original Asian range or stop/TP levels, so adopt
      // with a conservative stop (1% from current price) and no take profit.
      // The position will be closed at session end (16:00 London) regardless.
      const stopDistance = bp.averagePrice * 0.01;
      const stopLoss = bp.side === "buy"
        ? bp.averagePrice - stopDistance
        : bp.averagePrice + stopDistance;

      this.positions.set(bp.instrument, {
        instrument: bp.instrument,
        side: bp.side,
        entryPrice: bp.averagePrice,
        takeProfit: bp.side === "buy" ? Infinity : -Infinity,
        stopLoss,
        asianHigh: bp.averagePrice,
        asianLow: bp.averagePrice,
        asianRange: stopDistance,
        peakPnl: 0,
        peakPrice: bp.averagePrice,
        trailingActive: false,
      });

      console.log(
        `[london-breakout] Adopted existing ${bp.side} ${bp.units}u ${bp.instrument} @ ${bp.averagePrice} (stop=${stopLoss.toFixed(5)})`,
      );
    }
  }
  ...
  getState(): StrategyStateSnapshot {
    ...
    // Helper to format minutes remaining as "Xh Ym"
    const formatCountdown = (targetMin: number): string => {
      let diff = targetMin - londonMin;
    ...
    if (londonMin < 420) phase = `Asian Session (tracking range) — London open in ${formatCountdown(480)}`;
    else if (londonMin < 480) phase = `Pre-London (ranges locked) — open in ${formatCountdown(480)}`;
    else if (londonMin < 720) phase = this.positions.size > 0 ? "London Open (in position)" : "London Open (watching for breakout)";
    else if (londonMin < 960) phase = this.positions.size > 0 ? "London Session (holding)" : `London Session — closes in ${formatCountdown(960)}`;
    else phase = `Waiting for Asian Session — starts in ${formatCountdown(1440)}`;

    const indicators: StrategyIndicator[] = [];
    for (const inst of this.instruments) {
      const high = this.asianHighs.get(inst);
      const low = this.asianLows.get(inst);
      const currentPrice = this.prices.get(inst);
      if (!high || !low || high <= 0 || low === Infinity) continue;

      const range = high - low;
      const decimals = inst.includes("JPY") ? 3 : 5;
      let signal: StrategyIndicator["signal"] = "neutral";
      let detail = `H=${high.toFixed(decimals)} L=${low.toFixed(decimals)} R=${range.toFixed(decimals)}`;

      if (currentPrice && this.rangesLocked) {
        if (currentPrice > high) {
          signal = "buy";
          detail += ` | ${((currentPrice - high) / range * 100).toFixed(0)}% above`;
        } else if (currentPrice < low) {
          signal = "sell";
          detail += ` | ${((low - currentPrice) / range * 100).toFixed(0)}% below`;
        } else {
          detail += ` | in range`;
        }
      }

      indicators.push({ label: inst.replace("_", "/"), instrument: inst, value: detail, signal });
    }
    ...
    for (const [, pos] of this.positions) {
      ...
      positions.push({
        ...
        pnl,
        detail: `stop=${pos.stopLoss.toFixed(5)} peak=${pos.peakPnl.toFixed(5)} trail=${pos.trailingActive ? "on" : "off"}`,
      });
    }

    return {
      phase,
      detail: `${this.currentDate} | Ranges ${this.rangesLocked ? "locked" : "tracking"}`,
      indicators,
      positions,
    };
  }
